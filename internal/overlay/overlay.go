...
// Lifts the pixels inside r out of the captured outputs
func (s *Session) RegionImage(r Sel) *image.RGBA {
	w, h := r.x1-r.x0, r.y1-r.y0
	img := image.NewRGBA(image.Rect(0, 0, w, h))

	for _, o := range s.outs {
		gx0, gy0 := max(r.x0, o.x), max(r.y0, o.y)
		gx1, gy1 := min(r.x1, o.x+o.w), min(r.y1, o.y+o.h)
		if gx0 >= gx1 || gy0 >= gy1 {
			continue
		}
		...
		for gy := gy0; gy < gy1; gy++ {
			row := (gy - o.y) * o.stride
			for gx := gx0; gx < gx1; gx++ {
				pixelBGRA(img, gx-r.x0, gy-r.y0, o.orig, row+(gx-o.x)*4, o.format == shmARGB)
			}
		}
	}
...
func (c *conn) serveClipboard(mime string, data []byte) {
	...
	c.request(c.sourceID, 0, str(mime))        // source.offer
...
func (c *conn) sendClipboard(data []byte) {
	if len(c.fds) == 0 {
		return
	}
...
func Open() *Session {
	...
	return &Session{c: c, outs: outs, scVer: scVer}
}

func (s *Session) SelectRegion() (region Sel, confirmed bool) {
	region, confirmed = s.c.selectRegion(s.outs)

	s.c.fadeOut(s.outs)

	for _, o := range s.outs {
		s.c.request(o.id(roleLayer), 7)   // zwlr_layer_surface_v1.destroy
		s.c.request(o.id(roleSurface), 0) // wl_surface.destroy
...
func (s *Session) ServeClipboard(mime string, data []byte) {
	s.c.serveClipboard(mime, data)
}
